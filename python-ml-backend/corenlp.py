...
class StanfordCoreNLP:
    def __init__(self, path_or_host, port=None, memory='4g', lang='en', timeout=60000, quiet=True,
    ...
    def annotate_(self, text, properties=None):
        if sys.version_info.major >= 3:
            text = text.encode('utf-8')
        res = None
        try:
            r = requests.post(self.url, params={'properties': str(properties)}, data=text,
                          headers={'Connection': 'close'}, timeout=40)
            res = r.text
        except requests.exceptions.Timeout as e:
            pass
        return res

    def annotate(self, text, properties=None, doc_date=None):
        if sys.version_info.major >= 3:
           text = text.encode('utf-8')
        res = None
        try:
            if doc_date:
                params = {'properties': str(properties), 'date': str(doc_date)}
            else:
